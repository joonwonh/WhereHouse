...
setTotal <- function(){
  ...
  for(i in 1:nrow(total)){
    ...
    total.result[i,"score"] <- (total.result[i,"conv_store"]
                                ...
                                + total.result[i,"shop"]
                                + total.result[i,"cinema"] + 5);
  }
  #total.order <- order(total.result$score);
  #total.order
  
  for(i in 1:nrow(total.result)){
    if(total.result[i,"score"]<=30){
      total.result[i,"index"] <- 1;
    } else if(total.result[i,"score"]<=35){
      total.result[i,"index"] <- 2;
    } else if(total.result[i,"score"]<=45){
      total.result[i,"index"] <- 3;
    } else if(total.result[i,"score"]<=50){
      total.result[i,"index"] <- 4;
    } else if(total.result[i,"score"]<=55){
      total.result[i,"index"] <- 5;
    } else if(total.result[i,"score"]<=66){
      total.result[i,"index"] <- 6;
    } else if(total.result[i,"score"]<=79){
      total.result[i,"index"] <- 7;
    } else if(total.result[i,"score"]<=90){
      total.result[i,"index"] <- 8;
    } else {
      total.result[i,"index"] <- 9;
    }
...
min(total.result$score);
max(total.result$score);

mean(total.result$score);
sort(total.result$score);
...
#library(ggplot2)
color <- c("lightcoral","salmon","coral","orangered","tomato","indianred","red","firebrick", "darkred");
ggplot(total.result, aes(x=gu_name, y=index)) +
...
ggplot(total.result, aes(x=index)) +
