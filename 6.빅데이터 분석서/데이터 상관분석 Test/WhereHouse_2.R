...
setTotal <- function(){
  ...
  for(i in 1:nrow(total)){
    ...
    total.result[i,"conv_store"] <- total.result[i,"conv_store"] * 0.2;
    ...
    total.result[i,"cafe"] <- total.result[i,"cafe"] * 0.15;
    ...
    total.result[i,"cinema"] <- total.result[i,"cinema"] * 0.07;
    ...
    total.result[i,"park"] <- total.result[i,"park"] * 0.04;
...
ggplot(total.result, aes(x=total.result$index)) +
...
summary(model.total)
anova(model.total)
model.step <- stepAIC(model.total);
...
t.test(total.result[index==1,c("conv_store", "cafe")],total.result[index==2,c("conv_store", "cafe")])
